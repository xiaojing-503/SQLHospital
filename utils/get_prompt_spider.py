...
def create_table_prompt(
    ...
    formatted_table_fk = lambda table_fk: ",{table_fk}".format(
        table_fk=", ".join(
            [
                "{fk_columns_name} = {referenced_table_name}.{referenced_columns_name}".format(
    ...
    prompt = "\n".join(
        [
            "table = {table_name}, columns = [{formatted_columns}{formatted_table_pk}{formatted_table_fk}]".format(
...
def create_prompt(sample):
    ...
    prompt =  create_table_prompt(sample=sample, add_column_types=False, add_pk=False, add_fk=True)
